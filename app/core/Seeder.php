...
/**
 * Idempotent demo/dev seed — everything is INSERT IGNORE or keyed on a unique column, so
 * running it twice is harmless. NOT for production: passwords are a shared demo password.
 *
 * Shared by the CLI script (database/seed.php) and the Utilities page's "Import Demo
 * Database" action, so there is exactly one implementation. Callers on the web side must
 * gate this behind their own permission/environment checks — this class does not.
 */
final class Seeder
{
    ...
    private static function seed(): void
    {
        ...
        if (!function_exists('insertIgnore')) {
        ...
        $permissions = [
            ...
            ['platform.utilities.manage', 'platform'],
